...
Drev shares Claude Code session JSONLs through a Git repo. Commands run via the Bash tool. Drev must be on PATH; if not, suggest \`npm install -g @codeturion/drev\`.
...
- "drev: command not found": suggest \`npm install -g @codeturion/drev\`.
